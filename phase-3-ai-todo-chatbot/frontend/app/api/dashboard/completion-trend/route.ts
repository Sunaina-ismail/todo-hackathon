...
import { getCompletionTrend, formatChartData } from '@/lib/chart-utils'
...
export async function GET(request: Request) {
  try {
    ...
    const days = daysParam === '30' ? 30 : daysParam === '90' ? 90 : 7
    ...
    const trendData = getCompletionTrend(tasks, days)
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: 'INTERNAL_ERROR',
          message: 'Failed to fetch completion trend'
        }
      },
      { status: 500 }
    )
