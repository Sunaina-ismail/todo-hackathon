...
import type { Task, TaskCreate, TaskUpdate, TagWithUsage } from '@/types/task'
...
/**
 * Get authenticated user from Better Auth session and mint API JWT token
...
async function getAuthenticatedUser(): Promise<{ userId: string; token: string } | null> {
  try {
    ...
    // This JWT is signed with same secret backend uses for verification
    const claims = {
...
async function apiRequest<T>(
  ...
  const API_BASE_URL = process.env.BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8001'
...
/**
 * Fetch all tasks for authenticated user with search, filter, and sort support
 */
export async function fetchTasks(params?: {
  ...
  if (params?.sort_by) {
    ...
    // Convert simplified values to backend-compatible values
  ...
  // Extract tasks array and total count from response
...
export async function fetchTags(params?: {
  search?: string
  limit?: number
}): Promise<{ tags?: TagWithUsage[]; error?: string }> {
  const auth = await getAuthenticatedUser()
  ...
  const queryString = queryParams.toString()
  // Tags endpoint is at /api/{user_id}/tags
  const endpoint = `/api/${auth.userId}/tags${queryString ? `?${queryString}` : ''}`

  const result = await apiRequest<TagWithUsage[]>(endpoint, {
    method: 'GET',
  })
  ...
  // Tags come directly as array with usage_count
